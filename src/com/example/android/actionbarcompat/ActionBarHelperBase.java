 ...
 */
public abstract class ActionBarHelperBase {
    protected Activity mActivity;

    /**
     * Factory method for creating {@link ActionBarHelperBase} objects for a
     ...
     */
    public static ActionBarHelperBase createInstance(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            return new ActionBarHelperICS(activity);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            return new ActionBarHelperHoneycomb(activity);
        } else {
            return new ActionBarHelperCompat(activity);
        }
    }
